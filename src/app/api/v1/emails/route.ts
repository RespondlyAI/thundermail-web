...
export async function POST(request: NextRequest) {
  ...
  const { success: quotaLeft } = await emailSendingQuotaLimit.limit(apiKeyRecord.userId);
  if (!quotaLeft) {
  ...
  after(async () => {
    try {
      ...
      if (reqBody.event_type || reqBody.event_source) {
        ...
        const eventSource = reqBody.event_source || "api-v1";
      ...
      await sqsClient.send(new SendMessageCommand({ QueueUrl: process.env.AWS_SQS_QUEUE_URL, MessageBody: emailId }));
    } catch (e) {
      console.error("v1 background error:", e);
    }
  });

  return NextResponse.json({ id: emailId }, {
    status: 200,
    headers: { "ratelimit-limit": String(limit), "ratelimit-remaining": String(remaining), "ratelimit-reset": String(waitTime) }
  });
}
